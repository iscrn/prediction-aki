...
-- MEDICATIONS
DROP TABLE IF EXISTS MED_DCW24;
...
END ) AS NSAID
INTO MED_DCW24
FROM  DCW24H D JOIN MIMIC_UNTOUCHED.PRESCRIPTIONS P 
...
DROP TABLE IF EXISTS MED_DCW48;
SELECT D.ICUSTAY_ID, D.AKI_FLAG,
MAX(CASE
	WHEN P.drug ILIKE'%ethiodol%'OR P.drug ILIKE '%lipiodol%' 
		THEN 1 ELSE 0 
...
END ) AS NSAID
INTO MED_DCW48
FROM  DCW48H D JOIN MIMIC_UNTOUCHED.PRESCRIPTIONS P 
	ON D.ICUSTAY_ID = P.ICUSTAY_ID
WHERE DATE_TRUNC('DAY', P.ENDDATE ) >= DATE_TRUNC('DAY', D.INTIME ) 
...
DROP TABLE IF EXISTS MED_DCW72;
SELECT D.ICUSTAY_ID, D.AKI_FLAG,
MAX(CASE
	WHEN P.drug ILIKE'%ethiodol%'OR P.drug ILIKE '%lipiodol%' 
		THEN 1 ELSE 0 
...
END ) AS NSAID
INTO MED_DCW72
FROM  DCW72H D JOIN MIMIC_UNTOUCHED.PRESCRIPTIONS P 
	ON D.ICUSTAY_ID = P.ICUSTAY_ID
WHERE DATE_TRUNC('DAY', P.ENDDATE ) >= DATE_TRUNC('DAY', D.INTIME ) 
GROUP BY D.ICUSTAY_ID,  D.AKI_FLAG
ORDER BY D.ICUSTAY_ID;
...
DROP TABLE IF EXISTS MED_DCW96;
SELECT D.ICUSTAY_ID, D.AKI_FLAG,
MAX(CASE
	WHEN P.drug ILIKE'%ethiodol%'OR P.drug ILIKE '%lipiodol%' 
		THEN 1 ELSE 0 
...
END ) AS NSAID
INTO MED_DCW96
FROM  DCW96H D JOIN MIMIC_UNTOUCHED.PRESCRIPTIONS P 
	ON D.ICUSTAY_ID = P.ICUSTAY_ID
WHERE DATE_TRUNC('DAY', P.ENDDATE ) >= DATE_TRUNC('DAY', D.INTIME ) 
GROUP BY D.ICUSTAY_ID,  D.AKI_FLAG
ORDER BY D.ICUSTAY_ID;
...
DROP TABLE IF EXISTS MED_DCW120;
SELECT D.ICUSTAY_ID, D.AKI_FLAG,
MAX(CASE
	WHEN P.drug ILIKE'%ethiodol%'OR P.drug ILIKE '%lipiodol%' 
		THEN 1 ELSE 0 
...
END ) AS NSAID
INTO MED_DCW120
FROM  DCW120H D JOIN MIMIC_UNTOUCHED.PRESCRIPTIONS P 
	ON D.ICUSTAY_ID = P.ICUSTAY_ID
WHERE DATE_TRUNC('DAY', P.ENDDATE ) >= DATE_TRUNC('DAY', D.INTIME ) 
GROUP BY D.ICUSTAY_ID,  D.AKI_FLAG
ORDER BY D.ICUSTAY_ID;
...
DROP TABLE IF EXISTS MED_DCW144;
SELECT D.ICUSTAY_ID, D.AKI_FLAG,
MAX(CASE
	WHEN P.drug ILIKE'%ethiodol%'OR P.drug ILIKE '%lipiodol%' 
		THEN 1 ELSE 0 
...
END ) AS NSAID
INTO MED_DCW144
FROM  DCW144H D JOIN MIMIC_UNTOUCHED.PRESCRIPTIONS P 
	ON D.ICUSTAY_ID = P.ICUSTAY_ID
WHERE DATE_TRUNC('DAY', P.ENDDATE ) >= DATE_TRUNC('DAY', D.INTIME ) 
GROUP BY D.ICUSTAY_ID,  D.AKI_FLAG
ORDER BY D.ICUSTAY_ID;
...
-- COMORBIDITIES
DROP TABLE IF EXISTS COM_DCW24;
...
END) AS Jaundic
INTO COM_DCW24
FROM DCW24H D24 JOIN MIMIC_UNTOUCHED.DIAGNOSES_ICD D ON  D24.HADM_ID = D.HADM_ID
...
SELECT D.ICUSTAY_ID FROM DCW24H D 
WHERE D.ICUSTAY_ID NOT IN ( 
SELECT C.ICUSTAY_ID FROM COM_DCW24 C);
-- ICUSTAY_ID = 240408
...
DROP TABLE IF EXISTS COM_DCW48;
SELECT D24.ICUSTAY_ID, D24.AKI_FLAG, 
MAX(CASE 
    WHEN DC.ICD9_CODE = '4280' THEN 1 ELSE 0 
END )AS Congestive_heart_failure,
...
END) AS Jaundic
INTO COM_DCW48
FROM DCW48H D24 JOIN MIMIC_UNTOUCHED.DIAGNOSES_ICD D ON  D24.HADM_ID = D.HADM_ID
 	JOIN MIMIC_UNTOUCHED.D_ICD_DIAGNOSES DC ON D.ICD9_CODE = DC.ICD9_CODE
GROUP BY D24.ICUSTAY_ID, D24.AKI_FLAG
ORDER BY D24.ICUSTAY_ID;
--10573
...
DROP TABLE IF EXISTS COM_DCW72;
SELECT D24.ICUSTAY_ID, D24.AKI_FLAG, 
MAX(CASE 
    WHEN DC.ICD9_CODE = '4280' THEN 1 ELSE 0 
END )AS Congestive_heart_failure,
...
END) AS Jaundic
INTO COM_DCW72
FROM DCW72H D24 JOIN MIMIC_UNTOUCHED.DIAGNOSES_ICD D ON  D24.HADM_ID = D.HADM_ID
 	JOIN MIMIC_UNTOUCHED.D_ICD_DIAGNOSES DC ON D.ICD9_CODE = DC.ICD9_CODE
GROUP BY D24.ICUSTAY_ID, D24.AKI_FLAG
ORDER BY D24.ICUSTAY_ID;
--5107
...
DROP TABLE IF EXISTS COM_DCW96;
SELECT D24.ICUSTAY_ID, D24.AKI_FLAG, 
MAX(CASE 
    WHEN DC.ICD9_CODE = '4280' THEN 1 ELSE 0 
END )AS Congestive_heart_failure,
...
END) AS Jaundic
INTO COM_DCW96
FROM DCW96H D24 JOIN MIMIC_UNTOUCHED.DIAGNOSES_ICD D ON  D24.HADM_ID = D.HADM_ID
 	JOIN MIMIC_UNTOUCHED.D_ICD_DIAGNOSES DC ON D.ICD9_CODE = DC.ICD9_CODE
GROUP BY D24.ICUSTAY_ID, D24.AKI_FLAG
ORDER BY D24.ICUSTAY_ID;
--2837
...
DROP TABLE IF EXISTS COM_DCW120;
SELECT D24.ICUSTAY_ID, D24.AKI_FLAG, 
MAX(CASE 
    WHEN DC.ICD9_CODE = '4280' THEN 1 ELSE 0 
END )AS Congestive_heart_failure,
...
END) AS Jaundic
INTO COM_DCW120
FROM DCW120H D24 JOIN MIMIC_UNTOUCHED.DIAGNOSES_ICD D ON  D24.HADM_ID = D.HADM_ID
 	JOIN MIMIC_UNTOUCHED.D_ICD_DIAGNOSES DC ON D.ICD9_CODE = DC.ICD9_CODE
GROUP BY D24.ICUSTAY_ID, D24.AKI_FLAG
ORDER BY D24.ICUSTAY_ID;
--1809
...
DROP TABLE IF EXISTS COM_DCW144;
SELECT D24.ICUSTAY_ID, D24.AKI_FLAG, 
MAX(CASE 
    WHEN DC.ICD9_CODE = '4280' THEN 1 ELSE 0 
END )AS Congestive_heart_failure,
...
END) AS Jaundic
INTO COM_DCW144
FROM DCW144H D24 JOIN MIMIC_UNTOUCHED.DIAGNOSES_ICD D ON  D24.HADM_ID = D.HADM_ID
 	JOIN MIMIC_UNTOUCHED.D_ICD_DIAGNOSES DC ON D.ICD9_CODE = DC.ICD9_CODE
GROUP BY D24.ICUSTAY_ID, D24.AKI_FLAG
ORDER BY D24.ICUSTAY_ID;
--1261
...
DROP TABLE IF EXISTS DEM_DCW48;
SELECT ICUSTAY_ID, AKI_FLAG, GENDER, AGE, ETHNICITY
INTO DEM_DCW48
FROM DCW48H 
ORDER BY ICUSTAY_ID;
...
DROP TABLE IF EXISTS DEM_DCW72;
SELECT ICUSTAY_ID, AKI_FLAG, GENDER, AGE, ETHNICITY
INTO DEM_DCW72
FROM DCW72H 
ORDER BY ICUSTAY_ID;
...
DROP TABLE IF EXISTS DEM_DCW96;
SELECT ICUSTAY_ID, AKI_FLAG, GENDER, AGE, ETHNICITY
INTO DEM_DCW96
FROM DCW96H 
ORDER BY ICUSTAY_ID;
...
DROP TABLE IF EXISTS DEM_DCW120;
SELECT ICUSTAY_ID, AKI_FLAG, GENDER, AGE, ETHNICITY
INTO DEM_DCW120
FROM DCW120H 
ORDER BY ICUSTAY_ID;
...
DROP TABLE IF EXISTS DEM_DCW144;
SELECT ICUSTAY_ID, AKI_FLAG, GENDER, AGE, ETHNICITY
INTO DEM_DCW144
FROM  DCW144H 
ORDER BY ICUSTAY_ID;
...
DROP TABLE IF EXISTS BICARBONATE_24H;
...
INTO BICARBONATE_24H
FROM DCW24H D JOIN MIMIC_UNTOUCHED.LABEVENTS L on D.HADM_ID = L.HADM_ID
...
DROP TABLE IF EXISTS CHLORIDE_24H;
...
DROP TABLE IF EXISTS CREATININE_24H;
...
DROP TABLE IF EXISTS PLATELETS_24H;
...
DROP TABLE IF EXISTS POTASSIUM_24H;
...
DROP TABLE IF EXISTS CALCIUM_24H;
...
--48H LBEVENTS 
DROP TABLE IF EXISTS BICARBONATE_48H;
SELECT D.ICUSTAY_ID, ROUND(cast(AVG(L.VALUENUM)as NUMERIC),2) as AVG_BICARBONATE, MIN(L.VALUENUM) as MIN_BICARBONATE,
	MAX(L.VALUENUM) as MAX_BICARBONATE, COUNT(L.VALUENUM) as COUNT_BICARBONATE
INTO BICARBONATE_48H
FROM DCW48H D JOIN MIMIC_UNTOUCHED.LABEVENTS L on D.HADM_ID = L.HADM_ID
...
DROP TABLE IF EXISTS CHLORIDE_48H;
SELECT D.ICUSTAY_ID, ROUND(cast(AVG(L.VALUENUM)as NUMERIC),2) as AVG_CHLORIDE, MIN(L.VALUENUM) as MIN_CHLORIDE,
	MAX(L.VALUENUM) as MAX_CHLORIDE, COUNT(L.VALUENUM) as COUNT_CHLORIDE
INTO CHLORIDE_48H
FROM DCW48H D  JOIN MIMIC_UNTOUCHED.LABEVENTS L on D.HADM_ID = L.HADM_ID
...
DROP TABLE IF EXISTS CREATININE_48H;
SELECT D.ICUSTAY_ID, ROUND(cast(AVG(L.VALUENUM)as NUMERIC),2) as AVG_SCR, MIN(L.VALUENUM) as MIN_SCR,
	MAX(L.VALUENUM) as MAX_SCR, COUNT(L.VALUENUM) as COUNT_SCR
INTO CREATININE_48H
FROM DCW48H D JOIN MIMIC_UNTOUCHED.LABEVENTS L on D.HADM_ID = L.HADM_ID
...
DROP TABLE IF EXISTS PLATELETS_48H;
SELECT D.ICUSTAY_ID, ROUND(cast(AVG(L.VALUENUM)as NUMERIC),2) as AVG_PLATELETS, MIN(L.VALUENUM) as MIN_PLATELETS,
	MAX(L.VALUENUM) as MAX_PLATELETS, COUNT(L.VALUENUM) as COUNT_PLATELETS
INTO PLATELETS_48H
FROM DCW48H D  JOIN MIMIC_UNTOUCHED.LABEVENTS L on D.HADM_ID = L.HADM_ID
...
DROP TABLE IF EXISTS POTASSIUM_48H;
SELECT D.ICUSTAY_ID, ROUND(cast(AVG(L.VALUENUM)as NUMERIC),2) as AVG_POTASSIUM, MIN(L.VALUENUM) as MIN_POTASSIUM,
	MAX(L.VALUENUM) as MAX_POTASSIUM, COUNT(L.VALUENUM) as COUNT_POTASSIUM
INTO POTASSIUM_48H
FROM DCW48H D  JOIN MIMIC_UNTOUCHED.LABEVENTS L on D.HADM_ID = L.HADM_ID
...
DROP TABLE IF EXISTS CALCIUM_48H;
SELECT D.ICUSTAY_ID, ROUND(cast(AVG(L.VALUENUM)as NUMERIC),2) as AVG_CALCIUM, MIN(L.VALUENUM) as MIN_CALCIUM,
	MAX(L.VALUENUM) as MAX_CALCIUM, COUNT(L.VALUENUM) as COUNT_CALCIUM
INTO CALCIUM_48H
FROM DCW48H D  JOIN MIMIC_UNTOUCHED.LABEVENTS L on D.HADM_ID = L.HADM_ID
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- URINE 
drop table if exists URINES_DCW24;
...
INTO URINES_DCW24
FROM DCW24H F JOIN MIMIC_UNTOUCHED.OUTPUTEVENTS O on F.ICUSTAY_ID = O.ICUSTAY_ID --MODIFICATO
...
-- CHARTEVENTS
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS HEART_RATE_24H;
...
INTO HEART_RATE_24H
FROM DCW24H F  JOIN MIMIC_UNTOUCHED.CHARTEVENTS C on F.ICUSTAY_ID = C.ICUSTAY_ID
...
DROP TABLE IF EXISTS SYSBP_24H;
...
DROP TABLE IF EXISTS DIASBP_24H;
...
DROP TABLE IF EXISTS MEANBP_24H;
...
DROP TABLE IF EXISTS RESPRATE_24H;
...
DROP TABLE IF EXISTS GLUCOSE_24H;
...
DROP TABLE IF EXISTS ALL_LABDCW24;
...
INTO ALL_LABDCW24
FROM BUN_24H B JOIN BICARBONATE_24H BIC on B.ICUSTAY_ID = BIC.ICUSTAY_ID
	JOIN CHLORIDE_24H CH ON B.ICUSTAY_ID = CH.ICUSTAY_ID
	JOIN CREATININE_24H SCR ON B.ICUSTAY_ID = SCR.ICUSTAY_ID
	JOIN INR_24H I ON SCR.ICUSTAY_ID = I.ICUSTAY_ID
	JOIN PLATELETS_24H PL ON I.ICUSTAY_ID = PL.ICUSTAY_ID
	JOIN POTASSIUM_24H POT ON B.ICUSTAY_ID = POT.ICUSTAY_ID
	JOIN PTT_24H PTT ON POT.ICUSTAY_ID = PTT.ICUSTAY_ID
	JOIN WBC_24H WBC ON POT.ICUSTAY_ID = WBC.ICUSTAY_ID
	JOIN CALCIUM_24H C ON POT.ICUSTAY_ID = C.ICUSTAY_ID
	JOIN DCW24H W ON C.ICUSTAY_ID = W.ICUSTAY_ID
ORDER BY  W.ICUSTAY_ID;
--16479

-- CHECK FOR RANDOM MIN VALUES
SELECT * FROM ALL_LABDCW24
...
INTO LAB_DCW24
FROM ALL_LABDCW24 L JOIN DCW24H D ON L.ICUSTAY_ID = D.ICUSTAY_ID
ORDER BY D.ICUSTAY_ID;
--16479
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


DROP TABLE IF EXISTS ALL_CHARTDCW24;
...
INTO ALL_CHARTDCW24
FROM MEANBP_24H MBP JOIN HEART_RATE_24H HR on MBP.ICUSTAY_ID = HR.ICUSTAY_ID
	JOIN SYSBP_24H SBP ON MBP.ICUSTAY_ID = SBP.ICUSTAY_ID
	JOIN DIASBP_24H DBP ON MBP.ICUSTAY_ID = DBP.ICUSTAY_ID
	JOIN RESPRATE_24H R ON MBP.ICUSTAY_ID = R.ICUSTAY_ID
	JOIN TEMP_24H T ON MBP.ICUSTAY_ID = T.ICUSTAY_ID
	JOIN SPO2_24H SP ON MBP.ICUSTAY_ID = SP.ICUSTAY_ID
	JOIN GLUCOSE_24H G ON MBP.ICUSTAY_ID = G.ICUSTAY_ID
	JOIN DCW24H W ON G.ICUSTAY_ID = W.ICUSTAY_ID
ORDER BY W.ICUSTAY_ID;
--24702

--min_SPO2 and min_GLUCOSE have few missing values
...
INTO CHART_DCW24
FROM ALL_CHARTDCW24 C JOIN DCW24H D ON C.ICUSTAY_ID = D.ICUSTAY_ID
ORDER BY D.ICUSTAY_ID;
--24702
...
INTO BESTFEATURES24
FROM DEM24 D JOIN MED24 M ON D.ICUSTAY_ID = M.ICUSTAY_ID
	JOIN COM24 C ON M.ICUSTAY_ID = C.ICUSTAY_ID
	JOIN LAB24 L ON C.ICUSTAY_ID = L.ICUSTAY_ID
	JOIN CHART24 CH ON L.ICUSTAY_ID = CH.ICUSTAY_ID;
--15282
